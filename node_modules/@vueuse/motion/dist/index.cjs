...
function directive(variants, isPreset = false) {
  const register = (el, binding, node) => {
    ...
    const variantsObject = isPreset ? structuredClone(vue.toRaw(variants) || {}) : variants || {};
...
function setupMotionComponent(props) {
  ...
  const customPresets = vue.inject(CUSTOM_PRESETS, {});
  const preset = vue.computed(() => {
    if (props.preset == null) {
      return {};
    }
    if (customPresets != null && props.preset in customPresets) {
      return structuredClone(vue.toRaw(customPresets)[props.preset]);
    }
  ...
  setup(props) {
    ...
    return () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const n = nodes[i];
        if (n.type === vue.Fragment && Array.isArray(n.children)) {
...
exports.useMotionFeatures = useMotionFeatures;
